...
ODPT在線データを既定3秒ごとに取得し、駅到着・駅発車時点の遅延秒数をUbuntuのSQLiteへ保存する常駐サービスです。ブラウザが開かれていない時間も収集します。
...
- 既定で120日を過ぎた実績は運転日単位で自動削除します（`ATOS_HISTORY_RETENTION_DAYS`で変更可能）
- 保存フォルダは既定230GBを上限とし、95%に近づくと古い実績から削除して実効使用量を90%まで戻します（`ATOS_HISTORY_MAX_STORAGE_GB`で変更可能）
- 容量整理で生じたSQLite内の空き領域は新しい実績に再利用するため、収集を止めずに物理ファイルの増加を抑えます
...
`/health`では取得間隔、物理フォルダ容量、再利用可能容量、実効使用率、容量整理で削除した件数も確認できます。
