(function () {
  ...
  async function searchPeople() {
    ...
    try { const response=await api(`/v1/directory?q=${encodeURIComponent(query)}`);const people=(await response.json()).items;target.innerHTML=people.map(directoryRow).join("")||`<div class="directory-empty"><b>該当する利用者はいません</b><p>メールアドレスを確認してください。相手がまだ表示されない場合は、相手のアカウントで一度「撮影記録アーカイブ」へログインしてから再検索してください。</p></div>`; }
